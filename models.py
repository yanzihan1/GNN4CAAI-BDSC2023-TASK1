...
class CompGCN_DistMult(nn.Module):
    ...
    def forward(self, graph, sub, rel, dst=None,train=False,K=None):
        """
        :param g: dgl graph
        ...
        rel_emb = r_feats[rel, :]
        if train==True:
            random_choose_position=[i+20 for i in range(K)]
            rel_emb=rel_emb[1:]
            for n,_ in enumerate(rel_emb):
                if random.random()<0.5:
                    r=random.choice(random_choose_position)
                    t=rel_emb[n][r:r+10]
                    rel_emb[n][r:r+10]=rel_emb[n][r-10:r]
                    rel_emb[n][r-10:r]=t
